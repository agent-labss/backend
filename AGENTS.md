...
This repository currently has no commit history, so no project-specific commit convention is established. Use concise imperative subjects such as `Add readiness handler` or `Wire sqlite status checks`. Pull requests should include a short summary, tests run, linked issue or task context when available, and API or configuration changes such as new routes or environment variables.

## Security & Configuration Tips

Do not commit real database files, credentials, or service-account secrets. Keep secrets in environment variables and document new required settings near the code that reads them. Preserve readiness behavior so `/readyz` fails when the configured SQLite database is unavailable, while `/healthz` remains a lightweight process check.
